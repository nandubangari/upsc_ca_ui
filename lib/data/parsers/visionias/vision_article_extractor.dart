...
class VisionArticleExtractor implements BaseArticleExtractor {
  ...
  Future<List<ArticleContent>> fetchAndParse(String url) async {
    AppLogger.d('DEBUG: [Vision Extractor] Starting fetch for: $url');
    ...
    final h1V1 = document.querySelector('h1.text-3xl.text-center.text-pretty');
    final h1V2 = document.querySelector('h1.text-3xl');
    final h1V3 = document.querySelector('h1');
    ...
    AppLogger.d('DEBUG: [Vision Extractor] All H1s in doc: $allH1s');
    AppLogger.d('DEBUG: [Vision Extractor] H1 Selectors: v1=${h1V1?.text.trim()}, v2=${h1V2?.text.trim()}, v3=${h1V3?.text.trim()}');

    final mainTitle = h1V1?.text.trim() ?? 
                     h1V2?.text.trim() ??
                     h1V3?.text.trim() ??
                     'VisionIAS Article';

    AppLogger.d('DEBUG: [Vision Extractor] Final mainTitle chosen: $mainTitle');

    // 1. Find Container
    final container = document.querySelector('#article-content');
    if (container == null) {
      AppLogger.d('DEBUG: [Vision Extractor] #article-content not found');
    ...
    AppLogger.d('DEBUG: [Vision Extractor] Total small articles found: ${articleElements.length}');
    ...
    AppLogger.d('DEBUG: [Vision Extractor] Successfully extracted ${results.length} articles');
    return results;
  }
  ...
  ArticleContent _extractSingleArticle(dom.Element card, String baseUrl, String fallbackTitle, {bool forceFallback = false}) {
    ...
    if (forceFallback && !isH2Generic && h2Title != title) {
    ...
    final fullUrl = "${baseUrl.contains('#') ? baseUrl.split('#')[0] : baseUrl}#$id";
...
class VisionIASArticleExtractorContent {
  ...
  List<ListItem> _parseList(dom.Element listElement) {
    ...
    for (final li in listElement.children.where((e) => e.localName == 'li')) {
