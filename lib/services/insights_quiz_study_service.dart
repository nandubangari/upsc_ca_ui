...
class InsightsQuizStudyService {
  ...
  Future<List<DailyStudyData>> fetchForMonth(int year, int month, {DateTime? startDate}) async {
    ...
    try {
      ...
      print('DEBUG: [InsightsQuiz] Total quizzes fetched: ${allItems.length} (QUED: ${quedItems.length}, CA: ${caItems.length}, STATIC: ${staticItems.length}, DART: ${instaDartItems.length})');
      ...
      print('DEBUG: [InsightsQuiz] Matched $matchedMonthCount items for month $year/$month');
    } catch (e) {
      print('DEBUG: [InsightsQuiz] Error fetching for month $year/$month: $e');
    }
  ...
  Future<List<InsightsQuizItem>> _fetch(String url, String type) async {
    ...
    if (response.statusCode != 200) {
      print('DEBUG: [InsightsQuiz] Failed to fetch $type listing. Status: ${response.statusCode}');
      return [];
    }
